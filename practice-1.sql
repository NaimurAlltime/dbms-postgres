...
--question-1: inner join retrive employees and department information
...
--question-2: show department name and average salary
...
--question-3: count empolyee each department
SELECT department_name, count(employee_id) FROM employees
JOIN departments USING(department_id)
GROUP BY department_name;

--question-4: find the department heigst salary
SELECT department_name, round(avg(salary)) as avg_salary FROM employees
JOIN departments USING(department_id)
GROUP BY department_name
...
--count employee each hired date
